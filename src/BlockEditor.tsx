...
interface Props {
  initial: Pick<DrumBlock, 'startBar' | 'lengthBars'> & Partial<DrumBlock>;
...
export default function BlockEditor({ initial, onSave, onDelete, onClose }: Props) {
  ...
  function handleSave() {
    onSave({
      id: initial.id,
      startBar: initial.startBar,
      lengthBars: initial.lengthBars,
      color,
      label,
      note,
    });
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <h3>
          Bars {initial.startBar}
          {initial.lengthBars > 1 ? `–${initial.startBar + initial.lengthBars - 1}` : ''}
        </h3>
